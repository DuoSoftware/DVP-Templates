...
function CreateChatTemplate(req,res){
    ...
    if(req.body.content)
    {
        try {
            ...
            User.findOne({username: req.user.iss, company: company, tenant: tenant}, function (err, user) {
                ...
                if (err) {
                    ...
                    chatTemplateCategory.save(function (errTemp, resTemp) {
                        if (errTemp) {
                            jsonString = messageFormatter.FormatMessage(errTemp, "Error in creating chat templates", false, undefined);
                        }
                        else {
                            jsonString = messageFormatter.FormatMessage(undefined, "Chat template created successfully", true, resTemp);
                        }
            ...
            jsonString = messageFormatter.FormatMessage(e, "Exception found", false, undefined);
            res.end(jsonString);
        }
    }
    else
    {
        jsonString = messageFormatter.FormatMessage(new Error("No content found"), "No content found", false, undefined);
        res.end(jsonString);
    }
    ...
    User.findOne({username: req.user.iss, company: company, tenant: tenant}, function (err, user){
        ...
        else if(user)
        {
            ...
            ChatTemplate.find(queryObj,function (errTemp,resTemp) {
                if(errTemp)
                {
                    jsonString = messageFormatter.FormatMessage(errTemp, "Error in searching chat templates", false, undefined);
                    res.end(jsonString);
                }
                else if(resTemp)
                {
                    jsonString = messageFormatter.FormatMessage(undefined, "Chat templates found", true, resTemp);
                    res.end(jsonString);
                }
                else
                {
                    jsonString = messageFormatter.FormatMessage(undefined, "No chat templates found", true, resTemp);
                    res.end(jsonString);
                }
    ...
    User.findOne({username: req.user.iss, company: company, tenant: tenant}, function (err, user){
        ...
        else if(user)
        {
            ...
            ChatTemplate.find(queryObj,function (errTemp,resTemp) {
                if(errTemp)
                {
                    jsonString = messageFormatter.FormatMessage(errTemp, "Error in chat templates", false, undefined);
                    res.end(jsonString);
                }
                else (resTemp)
                {
...
function UpdateChatTemplate(req,res)
{
        ...
        if(req.body.content)
        {
            User.findOne({username: req.user.iss, company: company, tenant: tenant}, function (err, user){
                ...
                else if(user)
                {
                    ...
                    ChatTemplate.findOneAndUpdate(queryObj,{$set:{content:req.body.content}},function (errTemp,resTemp) {
                        if(errTemp)
                        {
                            jsonString = messageFormatter.FormatMessage(errTemp, "Error in searching chat templates", false, undefined);
                            res.end(jsonString);
    ...
    {
        jsonString = messageFormatter.FormatMessage(new Error("No template id found"), "No template id found", false, undefined);
        res.end(jsonString);
    }
