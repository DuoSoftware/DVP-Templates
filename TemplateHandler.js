...
function RenderTemplate (req,res)
{
    ...
    PickTemplate(HtmlTemplate,company,tenant, function (errPickTemp,resPickTemp) {
        if(errPickTemp)
        {
            jsonString=messageFormatter.FormatMessage(errPickTemp, "Template rendering failed", false, undefined);
            res.end(jsonString);
        }
        else
        {
            if(resPickTemp)
            {
                ...
                dust.render('hello', req.body.Parameters, function(errRendered, outRendered) {
                    if(errRendered)
                    {
                        console.log("Rendering Done");
                        jsonString=messageFormatter.FormatMessage(errRendered, "Template rendering failed", false, undefined);
                        res.end(jsonString);
                    }
                    else
                    {
            ...
            {
                console.log("Rendering Done");
                jsonString=messageFormatter.FormatMessage(new Error("Invalid template name"), "Invalid template name", fales, undefined);
                res.end(jsonString);
            }
...
function PickTemplateById(req,res)
{
    ...
    var company = parseInt(req.user.company);
    var tenant = parseInt(req.user.tenant);
    Template.findOne({_id:req.params.id,company:company,tenant:tenant},function (errPickTemplate,resPickTemplate) {
        ...
        else
        {
            ...
            {
                jsonString=messageFormatter.FormatMessage(new Error("No template found"), "Template picking failed", false, undefined);
            }
...
{
    Template.findOne({name:template,company:company,tenant:tenant},function (errPickTemplate,resPickTemplate) {
...
function RemoveTemplate(req,res)
{
    ...
    Template.findOneAndRemove({name:req.params.template,company:company,tenant:tenant}, function (errRemove,resRemove) {
        ...
        if(errRemove)
        {
            jsonString=messageFormatter.FormatMessage(errRemove, "Template deletion failed", false, undefined);
        }
        else
        {
            jsonString=messageFormatter.FormatMessage(undefined, "Template deletion succeeded", true, resRemove);
        }
        res.end(jsonString);
...
function UpdateTemplateContent(req,res)
{
    ...
    var company = parseInt(req.user.company);
    var tenant = parseInt(req.user.tenant);
    Template.findOne({_id:req.params.id,company:company,tenant:tenant}, function (errTemp,resTemp) {

        if(errTemp)
        {
            jsonString=messageFormatter.FormatMessage(errTemp, "Template selection failed", false, undefined);
            res.end(jsonString);
        }
        else
        {
            if(resTemp)
            {
                ...
                Template.findOneAndUpdate({_id:req.params.id,company:company,tenant:tenant},resTemp, function (errUpdate,resUpdate) {

                    if(errUpdate)
                    {
                        jsonString=messageFormatter.FormatMessage(errUpdate, "Template update failed", false, undefined);
                        res.end(jsonString);
                    }
                    else
                    {
                        jsonString=messageFormatter.FormatMessage(undefined, "Template update succeeded", true, resUpdate);
                        res.end(jsonString);
                    }
                });
            ...
            {
                jsonString=messageFormatter.FormatMessage(new Error("Template not found"), "Template not found", false, undefined);
                res.end(jsonString);
            }
...
function  AddTemplateStyles(req,res)
{
    ...
    for(var i=0;i<req.body.StyleFiles.length;i++)
    {
        ...
        if(i==req.body.StyleFiles.length-1)
        {
            ...
            Template.findOneAndUpdate({_id:req.params.id,company:company,tenant:tenant},{$addToSet:{styles:{$each:stylesObj}}}, function (errUpdate,resUpdate) {

                if(errUpdate)
                {
                    jsonString=messageFormatter.FormatMessage(errUpdate, "Styles replacing failed", false, undefined);
                    res.end(jsonString);
                }
                else
                {

                    jsonString=messageFormatter.FormatMessage(undefined, "Styles replacing doe", true, resUpdate);
                    res.end(jsonString);
                }
            });
        }
...
function  UpdateStyleContent(req,res)
{
    ...
    Template.findOneAndUpdate({_id:tempId,company:company,tenant:tenant,"styles._id":styleId},{"$set":{
        "styles.$.content":req.body.StyleContent

    }}, function (errStyle,resStyle) {
        if(errStyle)
        {
            jsonString=messageFormatter.FormatMessage(errStyle, "Styles searching failed", false, undefined);
            res.end(jsonString);
        }
        else
        {
            ...
            {
                jsonString=messageFormatter.FormatMessage(undefined, "Styles searching succeeded", true, resStyle);
                res.end(jsonString);
            }
...
function AddStyleToTemplate(req,res)
{
    ...
    Template.update({_id:req.params.id,company:company,tenant:tenant},{$addToSet:{styles:{$each:styleData}}}, function (errStyle,resStyle) {
        if(errStyle)
        {
            jsonString = messageFormatter.FormatMessage(errStyle, "Styles adding failed", false, undefined);
            res.end(jsonString);
        }
        else
        {
            jsonString=messageFormatter.FormatMessage(undefined, "Styles adding succeeded", true, resStyle);
            res.end(jsonString);
